...
def create_migrant(db: Session, migrant: schemas.MigrantCreate):
    migrant_data = migrant.dict()
    # Create PostGIS point if coordinates exist
    if migrant_data.get("location_lat") is not None and migrant_data.get("location_lng") is not None:
        migrant_data["location"] = func.ST_SetSRID(
            func.ST_MakePoint(migrant_data["location_lng"], migrant_data["location_lat"]), 
            4326
        )
    
    db_migrant = models.Migrant(**migrant_data)
    db.add(db_migrant)
    db.commit()
...
def update_migrant(db: Session, migrant_id: int, migrant_update: schemas.MigrantUpdate):
    ...
    if db_migrant:
        ...
        # Update location if coordinates are updated
        if "location_lat" in update_data or "location_lng" in update_data:
            lat = update_data.get("location_lat", db_migrant.location_lat)
            lng = update_data.get("location_lng", db_migrant.location_lng)
            if lat is not None and lng is not None:
                setattr(db_migrant, "location", func.ST_SetSRID(func.ST_MakePoint(lng, lat), 4326))
...
def create_encounter(db: Session, encounter: schemas.EncounterCreate, doctor_id: Optional[int] = None):
    ...
    # Create PostGIS point if coordinates exist
    if encounter_data.get("location_lat") is not None and encounter_data.get("location_lng") is not None:
        encounter_data["location"] = func.ST_SetSRID(
            func.ST_MakePoint(encounter_data["location_lng"], encounter_data["location_lat"]), 
            4326
        )
